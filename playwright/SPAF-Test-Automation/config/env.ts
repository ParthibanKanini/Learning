...
function loadEnv(): AppEnv {
  ...
  logger.info(`<<<<< password for user ${cfg.defaultUser.password} from base64`);

  // Decode base64 password from config file if it looks like base64
  if (cfg.defaultUser?.password && cfg.defaultUser.password.match(/^[A-Za-z0-9+/=]+$/)) {
    try {
      cfg.defaultUser.password = Buffer.from(cfg.defaultUser.password, 'base64').toString('utf-8');
    } catch {
      // If decoding fails, use password as-is
      log.info('Password does not appear to be base64 encoded, using as-is');
    }
    logger.info(`<<<<< Decoded password for user ${cfg.defaultUser.password} from base64`);
  }
