...
/**
 * Handles logging of JVM memory and system statistics. Provides formatted table output for various
 * monitoring metrics.
 */
public class MemoryStatsLogger {
  ...
  public void logGarbageCollection(GarbageCollectionStats gcStats) {
    ...
    if (gcStats.totalCollectionCount > 0) {
      logger.info(
          "GC Total: Collections={} | Total Time={} ms",
          gcStats.totalCollectionCount,
          gcStats.totalCollectionTimeMs);
    }
  }
  ...
  public void logThreads(ThreadStatistics threadStats) {
    ...
    if (threadStats.deadlockedThreadCount > 0) {
      logger.warn(
          "DEADLOCK DETECTED: {} threads are deadlocked!", threadStats.deadlockedThreadCount);
    }
  }
  ...
  public void logSystem(SystemStatistics sysStats) {
    logger.info(
        "System: Available Processors={} | System Load Average={}",
        sysStats.availableProcessors,
        sysStats.systemLoadAverage >= 0
            ? String.format("%.2f", sysStats.systemLoadAverage)
            : "Not available");

    if (sysStats.physicalMemory != null) {
      logger.info(
          "Physical Memory: Total={} MB | Used={} MB | Free={} MB | Usage={} %",
    ...
    if (sysStats.swapSpace != null) {
      logger.info(
          "Swap Space: Total={} MB | Used={} MB | Free={} MB",
    ...
    if (sysStats.cpuLoad != null) {
      logger.info(
          ...
          sysStats.cpuLoad.processPercent >= 0
              ? String.format("%.2f", sysStats.cpuLoad.processPercent * 100)
              : "Not available",
          sysStats.cpuLoad.systemPercent >= 0
              ? String.format("%.2f", sysStats.cpuLoad.systemPercent * 100)
              : "Not available");
    }
    ...
    if (sysStats.committedVirtualMemoryMB > 0) {
      logger.info(
          ...
          String.format("%.2f", sysStats.committedVirtualMemoryMB));
    }

    if (sysStats.uptime != null) {
      logger.info(
          "JVM Uptime: {} ms ({} minutes, {} seconds)",
          sysStats.uptime.uptimeMs,
          sysStats.uptime.uptimeMinutes,
          sysStats.uptime.uptimeSeconds % 60);
    }
  }
  ...
  public void logClassLoading(ClassLoadingStats classStats) {
    logger.info(
        "Classes: Loaded={} | Total Loaded={} | Unloaded={}",
        classStats.loadedClassCount,
        classStats.totalLoadedClassCount,
        classStats.unloadedClassCount);
  }
}
