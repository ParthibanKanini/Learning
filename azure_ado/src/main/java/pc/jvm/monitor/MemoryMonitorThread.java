...
/**
 * A daemon thread that periodically monitors and logs JVM memory and system statistics.
 *
 * <p>This thread provides detailed insights into: - Heap memory usage (used, committed, max) -
 * Non-heap memory usage (Metaspace, Code Cache, etc.) - Garbage collection (collection counts and
 * times) - Thread count and states - System stats (CPU load, physical memory, swap space, - Memory
 * pool details
 */
public class MemoryMonitorThread extends Thread {
  ...
  private void logMemoryStats(String logType) {
    ...
    statsLogger.logMemoryUsage(
        "HEAP",
        stats.heapMemory.initMB,
        stats.heapMemory.usedMB,
        stats.heapMemory.committedMB,
        stats.heapMemory.maxMB,
        stats.heapMemory.usagePercent);
    ...
    statsLogger.logMemoryUsage(
        "NON-HEAP",
        -1,
        stats.nonHeapMemory.usedMB,
        stats.nonHeapMemory.committedMB,
        stats.nonHeapMemory.maxMB,
        stats.nonHeapMemory.usagePercent);
  ...
  /**
   * Collects non-heap memory statistics. Non-heap memory is allocated outside the Java heap from
   * native OS memory.
   *
   * <p>Non-heap memory includes: - Metaspace: Class metadata, method data, runtime constant pool
   * (replaced PermGen in Java 8+) - Code Cache: JIT-compiled native code - Thread stacks: Native
   * memory for thread call stacks - JVM Internal structures: Symbol table, JNI data structures - GC
   * internal data structures
   *
   * <p>Most non-heap memory is not managed by GC, except Metaspace which can be garbage collected
   * when classes are unloaded.
   */
  ...
  private GarbageCollectionStats collectGarbageCollectionStats() {
    ...
    for (GarbageCollectorMXBean gcBean : gcBeans) {
      ...
      if (collector.collectionCount > 0) {
        collector.avgCollectionTimeMs =
            collector.collectionTimeMs / (double) collector.collectionCount;
      }
  ...
  private ThreadStatistics collectThreadStats() {
    ...
    for (long threadId : threadIds) {
      ...
      if (threadInfo != null) {
        ...
        switch (state) {
          case RUNNABLE -> stats.states.runnable++;
          case BLOCKED -> stats.states.blocked++;
          case WAITING -> stats.states.waiting++;
          case TIMED_WAITING -> stats.states.timedWaiting++;
          case NEW -> stats.states.newState++;
          case TERMINATED -> stats.states.terminated++;
        }
      }
    }
